...
def main(args):
    ...
    path2 = f"{args.save}/mulog_mcmc"
    path3 = f"{args.save}/random"
    path4 = f"{args.save}/mulog_added_batch"
    path5 = f"{args.save}/mulog_mcmc_batch"
    path6 = f"{args.save}/random_batch"
    ...
    for i, (x, _) in enumerate(tqdm(test_queue, total=200)):
        ...
        z, mu, log_var = netE(x)
        # x = torch.zeros(B, 3, W, H)
        eps_z1 = torch.randn(B, netG.nz, device=device, requires_grad=True)
        #eps_z1 = torch.zeros(B, netG.nz, device=device, requires_grad=True)
        initial_zs.append(eps_z1.clone().detach().cpu().numpy())
        eps_z2 = z.squeeze().detach().clone().requires_grad_()
        
        eps_zs = [eps_z1]
        

        for index, eps_z in enumerate(eps_zs):
            eps_x = torch.randn_like(x, device=device, requires_grad=True)
            ...
            if index == 0:
                ...
                #final_random_zs.append(np.random.normal(size=(16,200)))

                dist_xz_random = netG(eps_z.reshape(B, netG.nz, 1, 1))
                final_sample= dist_xz_random.dist.mu
                grid = make_grid(final_sample, nrow=4, pad_value=1)
                # save_image(grid, f"{args.save}/random_batch/{i}.jpg", normalize=True)
                # for j in range(B):
                #     save_image(final_sample[j], f"{args.save}/random/{i*B+j}.jpg", normalize=True)
                
                dist_xz_mulog_added = netG(mu + eps_z.reshape(B, netG.nz, 1, 1) * torch.exp(log_var))
                final_sample= dist_xz_mulog_added.dist.mu
                grid = make_grid(final_sample, nrow=4, pad_value=1)
                # save_image(grid, f"{args.save}/mulog_added_batch/{i}.jpg", normalize=True)
                # for j in range(B):
                #     save_image(final_sample[j], f"{args.save}/mulog_added/{i*B+j}.jpg", normalize=True)
            else:
                dist_xz_mulog_mcmc = netG(eps_z.reshape(B, netG.nz, 1, 1))
                final_sample = dist_xz_mulog_mcmc.dist.mu
                grid = make_grid(final_sample, nrow=4, pad_value=1)
                # save_image(grid, f"{args.save}/mulog_mcmc_batch/{i}.jpg", normalize=True)
                # for j in range(B):
                #     save_image(final_sample[j], f"{args.save}/mulog_mcmc/{i*B+j}.jpg", normalize=True)

    initial_zs = np.concatenate(initial_zs, axis=0)
    final_random_zs = np.concatenate(final_random_zs, axis=0)
    print(final_random_zs.shape)
    ...
    print(f'Avg mcmc dist = {np.mean(np.linalg.norm(initial_zs-final_random_zs, axis=1))}')
