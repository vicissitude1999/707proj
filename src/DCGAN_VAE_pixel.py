...
class DCGAN_G(nn.Module):
    ...
    def forward(self, input, in_vae=True):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            output = nn.parallel.data_parallel(self.main, input, range(self.ngpu))
            output = output.view(-1, self.nc, self.isize, self.isize)
        else: 
            output = self.main(input)
            output = output.view(-1, self.nc, self.isize, self.isize)
        ...
        # return output
        if in_vae:
            output = torch.cat((output, math.log(1) * torch.ones_like(output, device=output.device)), dim=1)
        else:
            output = torch.cat((output, math.log(0.1) * torch.ones_like(output, device=output.device)), dim=1)
        # print(output.shape)
        dist_xz = distributions.NormalDecoder(param=output)
